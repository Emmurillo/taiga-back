...
def test_update_color_tag(client, data):
    users = [
        None,
        data.registered_user,
        data.project_member_without_perms,
    ...
    url = reverse('projects-update-color-tag', kwargs={"pk": data.public_project.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [401, 403, 403, 403, 200]

    url = reverse('projects-update-color-tag', kwargs={"pk": data.private_project1.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [401, 403, 403, 403, 200]

    url = reverse('projects-update-color-tag', kwargs={"pk": data.private_project2.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [404, 404, 404, 403, 200]

    url = reverse('projects-update-color-tag', kwargs={"pk": data.blocked_project.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [404, 404, 404, 403, 451]


def test_rename_tag(client, data):
    users = [
        None,
        data.registered_user,
        data.project_member_without_perms,
    ...
    url = reverse('projects-rename-tag', kwargs={"pk": data.public_project.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [401, 403, 403, 403, 200]

    url = reverse('projects-rename-tag', kwargs={"pk": data.private_project1.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [401, 403, 403, 403, 200]

    url = reverse('projects-rename-tag', kwargs={"pk": data.private_project2.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [404, 404, 404, 403, 200]

    url = reverse('projects-rename-tag', kwargs={"pk": data.blocked_project.pk})
    results = helper_test_http_method(client, 'post', url, post_data, users)
    assert results == [404, 404, 404, 403, 451]
